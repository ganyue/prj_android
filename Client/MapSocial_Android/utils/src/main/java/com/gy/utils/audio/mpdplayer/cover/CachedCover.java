...
public class CachedCover implements ICoverRetriever {
    ...
    public CachedCover(Application context) {
        if (context == null)
    ...
    public void save(AlbumInfo albumInfo, Bitmap cover) {
        if (!Environment.MEDIA_MOUNTED.equals(Environment.getExternalStorageState())) {
            ...
            e("wl", "No writable external storage, not saving cover to cache");
            return;
        }
        ...
        try {
            ...
            e("wl", "Cache cover write failure : " + e);
        } finally {
            if (out != null) {
                try {
                    out.close();
                } catch (IOException e) {
                    e("wl", "Cannot close cover stream : " + e);
                }
            }
        }
    }
    ...
    public void delete(AlbumInfo albumInfo) {
        ...
        if (files != null) {
            for (File f : files) {
                // No need to take care of subfolders, there won't be any.
                // (Or at least any that MPDroid cares about)
                if (CoverManager.getCoverFileName(albumInfo).equals(f.getName())) {
